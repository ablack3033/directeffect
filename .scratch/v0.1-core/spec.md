...
- **Two engines, one model.** The frequentist engine wraps netmeta (mapping estimate→TE, std_error→seTE, target→treat1, comparator→treat2, study_id→studlab; common and random summaries requested, common used in v0.1). The Bayesian engine implements the identical likelihood in Stan with a weak `normal(0, 5)` prior on free effects so comparison against netmeta is meaningful. Each engine lives behind an adapter; netmeta and Stan internals are never exposed outside their adapters. *(Precision added by v0.1-uncertainty-fixes: the likelihood is identical for networks with one comparison per study. Multi-arm trials are supported by the netmeta engine only — netmeta models the within-study correlation, and the Stan engine refuses such networks rather than treat correlated rows as independent.)*
